...
__includes["prestige.nls" "vertical.nls" "unbiased.nls" "conformist.nls" "simpson_diversity.nls" "move_bounded.nls"]

;; define variables
globals [ prestigeTrait simpson outFile stopnow] 
  ;; prestigeTrait: the index number of the trait that determines prestige (cannot be larger than traitRange), 
  ;; simpson: Simpson Diversity Index, 
  ;; outfile: name and path of output csv file, 
  ;; stopnow: stop simulation flag

; INTERFACE GLOBALS
  ;; nTraits: number of traits an agent can possess, 
  ;; traitRange: number of values that a trait can have,
  ;; rate at which agents reproduce (an agent will die for each new one hatched), 
  ;; moveDistance: distance an agent moves each time step,
...
to setup
  ...
  clear-all
  set stopnow false
  ;set nTraits 3 ;; number of traits possessed by each agent. Stored as places in the "traits" list for each agent.
  set prestigeTrait 0 ;; index of item in "traits" list that deterimines an agent's prestige
...
to set-color
  ...
  let maxColorValue nTraits * traitRange
  let traitSum 0
  foreach traits [set traitSum ? + traitSum]
  set color round (140 * traitSum / maxColorValue)
  if color mod 10 = 0 [set color color + 2] ;; avoid black colors
end

to go
  ask agents [
    move-bounded moveDistance ;; call move procedure for each agent
    ...
    if transmissionType = "vertical" [
      ;; call vertical transmission procedure, where information is transmitted only to offspring
      let offspring vertical traitRange innovationRate replacementRate
      if offspring != nobody [
        ;; a new offspring is hatched
        ask offspring [set-color] ;; set agent color to indicate new cultural knowledge if there is innovation/copy error
        ] 
      ] 
    
    if transmissionType = "unbiased" [
      ;; call horizontal transmission procedure, where information it transmitted from another agent encountered
      unbiased traitRange interactionRadius innovationRate
      set-color
      ]
    
    if transmissionType = "prestige" [
      ;; call horizontal transmission procedure, where probability of information transmission is determined 
      ;; by value in prestige trait of another agent encountered
      prestige traitRange interactionRadius innovationRate prestigeTrait 
      set-color
      ] 

    if transmissionType = "conformist" [
      ;; call horizontal transmission procedure, where information transmission is determined 
      ;; by the popularity of a trait value among agents within "interactionRadius" of calling agent
      conformist traitRange interactionRadius innovationRate 
      set-color
      ] 
  ]
  ...
  if timeSteps > 0 and ticks >= timeSteps [ 
    ;; do summary statistics and stop simulation if time steps exceed timeSteps value
    set simpson simpson-diversity
    set stopnow true ;; set stop flag
    ]  
  if stopnow [stop] ;; stop simulation
end
